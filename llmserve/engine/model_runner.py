...
class ModelRunner:
    ...
    def _speculative(self):
        executor = getattr(self, "speculative_executor", None)
        if executor is None:
            executor = SpeculativeExecutor(self)
            self.speculative_executor = executor
        return executor

    # Keep these entry points on ModelRunner for scheduler, IPC, and test compatibility.
    def load_draft_model(self, *args, **kwargs):
        return self._speculative().load_draft_model(*args, **kwargs)

    def _draft_prefill(self, *args, **kwargs):
        return self._speculative()._draft_prefill(*args, **kwargs)

    def _accumulate_draft_prefill(self, *args, **kwargs):
        return self._speculative()._accumulate_draft_prefill(*args, **kwargs)

    def _get_single_draft_kv(self, *args, **kwargs):
        return self._speculative()._get_single_draft_kv(*args, **kwargs)

    def clear_speculative_state(self, *args, **kwargs):
        return self._speculative().clear_speculative_state(*args, **kwargs)

    def _draft_stage_timing(self, *args, **kwargs):
        return self._speculative()._draft_stage_timing(*args, **kwargs)

    def _update_single_draft_kv(self, *args, **kwargs):
        return self._speculative()._update_single_draft_kv(*args, **kwargs)

    def _fill_prefill_sampled_tokens(self, *args, **kwargs):
        return self._speculative()._fill_prefill_sampled_tokens(*args, **kwargs)

    def _build_speculative_debug(self, *args, **kwargs):
        return self._speculative()._build_speculative_debug(*args, **kwargs)

    def run_target_decode_with_eagle3_aux(self, *args, **kwargs):
        return self._speculative().run_target_decode_with_eagle3_aux(*args, **kwargs)
    ...
    def _generate_speculative_draft_sequences(self, *args, **kwargs):
        return self._speculative()._generate_speculative_draft_sequences(*args, **kwargs)

    def run_speculative_batch(self, *args, **kwargs):
        return self._speculative().run_speculative_batch(*args, **kwargs)
