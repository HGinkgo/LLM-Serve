...
- Batched EAGLE draft proposal 与 packed variable-length target verify：合批生成和验证，同时保持每个请求的 KV 长度、接受与回退相互独立。
...
进行可复现的 draft batching 消融时，两组实验都加 `--argmax-sampler`，serial draft 组额外加 `--disable-batched-draft`。默认仍启用 batched draft proposal。
