...
Stage 4 与 SchedulerOutput 重构后的 Stage 5 各包含三类 workload、18 次正式运行、汇总 CSV 和脱敏代表性 JSON，见 [`benchmarks/results/`](benchmarks/results/)。Stage 5 的 decode-heavy EAGLE 吞吐为 baseline 的 `1.327x`，mixed steady-state 为 `1.105x`。
